...
/**
 * |---Map:双列数据，存储 key-value对的数据 ---类似于高中的函数：y = f(x)
 ...
 *          如果key1的哈希值与已经存在的某一个数据的哈希值相同，继续比较:调用key1所在类的equals(key2)方法，比较：
 *              如果equals()返回的是false：此时key1-value1添加成功。 ---情况3
 *              如果equals()返回的是true：使用value1替换value2.
 *
 *      补充：关于情况2和情况3：此时key1-value1和原来的数据以链表的方式存储。
 *
 *      在添加过程中，会涉及到扩容问题，默认的扩容方式：扩容为原来的容量的2倍，并将原有的数复制过来。
 *
 *      jdk 8相较于jdk 7在底层实现方面的不同。
 *      1.new HashMap():底层没有创建一个长度为16的数组
 *      2.jdk 8底层数组是：Node[]，而非Entry[]
 *      3.首次调用put()方法时，底层创建长度为16的数组
 *      4.jdk 7地层结构只有：数组+链表。jdk 8中底层结构：数组+链表+红黑树。
 *        当数据的某一个索引位置上的一个元素以链表形式存在的个数 > 8 且当前的数组长度 > 64时，
